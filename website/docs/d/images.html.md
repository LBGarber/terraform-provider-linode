...
* `name` - (Required) The name of the field to filter by. See the [Linode API Documentation](https://www.linode.com/docs/api/images/#images-list__responses) for a list of filterable fields.

* `values` - (Required) A list of values for the filter to allow. These values should all be in string form.

## Attributes

Each Linode image will be stored in the `image` attribute and will export the following attributes:
