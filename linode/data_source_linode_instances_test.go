		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resName, "linode.#", "1"),
					resource.TestCheckResourceAttr(resName, "linode.0.type", "g6-nanode-1"),
					resource.TestCheckResourceAttr(resName, "linode.0.tags.#", "2"),
					resource.TestCheckResourceAttr(resName, "linode.0.image", "linode/ubuntu18.04"),
					resource.TestCheckResourceAttr(resName, "linode.0.region", "us-east"),
					resource.TestCheckResourceAttr(resName, "linode.0.group", "tf_test"),
					resource.TestCheckResourceAttr(resName, "linode.0.swap_size", "256"),
					resource.TestCheckResourceAttr(resName, "linode.0.ipv4.#", "2"),
					resource.TestCheckResourceAttrSet(resName, "linode.0.ipv6"),
					resource.TestCheckResourceAttr(resName, "linode.0.disk.#", "2"),
					resource.TestCheckResourceAttr(resName, "linode.0.config.#", "1"),
				),
			},
		},
	})
...
func testAccDataSourceCheckLinodeAllInstancesNotEmpty(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		linodeCount, err := strconv.Atoi(rs.Primary.Attributes["linode.#"])
		if err != nil {
